...
-- Fix the process_transaction_reversal function to properly create reversal transactions
...
BEGIN
    -- Get the reversal request (don't check status here since we're updating it to approved)
    ...
    RAISE NOTICE 'Created reversal transaction % for original transaction %', new_txn_id, original_txn.id;
